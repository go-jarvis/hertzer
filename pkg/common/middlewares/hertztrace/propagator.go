...
func Inject(ctx context.Context, cfg *config, headers *protocol.RequestHeader) {
	cfg.propagator.Inject(ctx, &metadataProvider{headers: headers})
}

// Extract returns the baggage and span context
func Extract(ctx context.Context, cfg *config, headers *protocol.RequestHeader) (baggage.Baggage, trace.SpanContext) {
	ctx = cfg.propagator.Extract(ctx, &metadataProvider{headers: headers})
